...
## 54. Outils compagnons — vague jcode (v0.9.7)

Cinq primitives additives empruntées à [jcode](https://github.com/1jehuang/jcode) atterrissent en parallèle d'une surface CLI étendue, pour que SuperAgent et SuperAICore partagent le même vocabulaire d'expérience opérateur. Aucun de ces ajouts ne change les shapes d'appel SDK ni le dispatch core — chaque pièce est opt-in et dégrade en no-op quand le câblage hôte est absent.

### `agent_grep` — grep token-aware avec troncature « chunk vu »

Frère de l'outil `grep` historique (qui est byte-pour-byte aligné avec ripgrep). `AgentGrepTool` ajoute deux choses sur la même surface de flags :

1. **Injection du symbole englobant.** Chaque hit vient avec le nom de la fonction / classe / méthode qui contient physiquement la ligne matchée — le modèle voit `MyClass::handle()` plutôt qu'un brut « ligne 42 de MyClass.php ».
2. **Troncature « chunk déjà vu » par session.** Quand le même triplet `(file, lineRange, sha)` réapparaît dans un set de résultats déjà servi au modèle, l'outil court-circuite avec un marqueur au lieu de réémettre le hunk — pour empêcher « relire le même morceau trois tours d'affilée ». L'état vit dans `ToolStateManager`, keyé par `(file, lineRange, sha)`, donc l'isolation swarm fonctionne sans qu'un agent ne fuite son ledger « déjà vu » à un autre.

```php
use SuperAgent\Tools\Builtin\AgentGrepTool;

$agent->loadTools(['grep']);                  // alignement ripgrep byte-à-byte
$agent->registerTool(new AgentGrepTool());    // frère, sortie augmentée

// Par appel : choix de la shape selon le besoin.
$agent->run('trouve tous les appelants de MyClass::handle et donne-moi la méthode englobante');
```

### SPI d'extraction de symboles — `Tools\Builtin\Symbols\SymbolExtractor`

La résolution du symbole englobant derrière `AgentGrepTool` est branchable. Trois implémentations de référence :

| Classe | Mode | Langages | Précision | Coût |
|---|---|---|---|---|
| `RegexSymbolExtractor` | Pur PHP | PHP / JS / TS / Python / Go | ~95 % sur du code typique | Toujours dispo, zéro I/O |
| `TreeSitterSymbolExtractor` | shell vers le CLI `tree-sitter` | ~15 grammaires (PHP / JS / TS / Python / Go / Rust / Ruby / Java / C / C++ / C# / Swift / Kotlin / Scala / Lua) | Niveau parser | Sous-process + timeout 1,5 s |
| `CompositeSymbolExtractor` | Chaîne d'extracteurs | Premier non-vide gagne | Le meilleur des deux | Premier succès court-circuite |
...
$tool = new AgentGrepTool(symbolExtractor: new CompositeSymbolExtractor([
    new TreeSitterSymbolExtractor(),     // tenté d'abord ; passe au suivant si échec
    new RegexSymbolExtractor(),          // réussit toujours
]));
```

`TreeSitterSymbolExtractor` auto-détecte le binaire `tree-sitter` sur `$PATH`, surchargeable via `SUPERAGENT_TREE_SITTER_BIN` ou un argument constructeur. Il mappe les hints de langage SuperAgent vers la grammaire correspondante, parse la sortie s-expression sous timeout dur (1,5 s par défaut). Binaire absent / grammaire non enregistrée / invocation foirée — dégrade en « je ne supporte pas » via `supports()` ; ne lève jamais.

### Swarm — `FileLedger` + `FileShiftedEvent`

Le problème de collision d'édition cross-agent : l'agent A écrit un fichier que l'agent B a déjà lu, et la prochaine décision de B se fait désormais sur du contenu périmé. `FileLedger` track les lectures et écritures ; sur une écriture, il émet un `FileShiftedEvent` à chaque agent qui avait déjà lu le même path.

```php
use SuperAgent\Swarm\FileLedger;
use SuperAgent\Swarm\Events\FileShiftedEvent;

$ledger = $worktreeManager->fileLedger();    // paresseux, attaché au premier appel
...
// Les outils qui lisent/écrivent opt-in en appelant ces méthodes :
...
$ledger->recordWrite(
    ...
    shaBefore: '...',
    shaAfter: '...',
    summary: 'corrige le garde null dans foo()',
);
// emitter déclenche une fois avec toAgent=$agentB

// Hooks de cleanup :
$collisions = $ledger->peerCollisions($agentB);   // pour inspection
$ledger->clearAgent($agentB);                     // au shutdown d'un agent
```

**Calibrage.** Dimensionné pour des swarms in-process (dizaines d'agents, centaines de fichiers en vol). Les déploiements plus larges branchent un emitter custom qui forwarde les events vers Redis pub/sub ou des websockets — le callback de l'emitter est le seul échange.

**Emitter par défaut no-op.** Les swarms existants qui n'opt-in pas leurs outils sur `recordRead/recordWrite` restent byte-compat.

### Swarm — `AmbientWorker`

Worker longue durée, basse priorité, qui exécute la dédup mémoire + scans de péremption à chaque tick. Les hôtes appellent `tick()` depuis une boucle (cron, swoole, react, simple `while sleep`) ; chaque pass fait respecter son propre `pass_budget_seconds`, donc un tick ne bloque jamais plus que quelques secondes. Le coût en tokens est tagué `usage_source: 'ambient'` via le callback fourni — les dashboards séparent dépense user-facing et arrière-plan (le CostDashboard de SuperAICore rend une slice « ambient » sans plomberie supplémentaire).
...
Le worker fait respecter le budget par pass en interne, donc un tick ne bloque jamais plus que quelques secondes — la boucle hôte choisit seulement *à quelle fréquence* tick, pas *combien de temps* un tick dure.

### Skills — montée de version `SemanticSkillRouter`

Le router qui pioche les top-K skills par cosinus d'embedding avant de les injecter dans le system prompt. Accepte désormais soit une instance `EmbeddingProvider`, soit la shape originale `callable(list<string>): list<list<float>>` (auto-wrappée via `CallableEmbeddingProvider`). Retombe sur le chevauchement de mots-clés quand rien n'est fourni. Réutilise les vecteurs en cache keyés par hash du contenu skill, donc le re-embedding ne se produit que quand la description d'un skill change.
...
// Production : vrai embedder
$router = new SemanticSkillRouter(
    embedder: new OllamaEmbeddingProvider(),
    topK: 5,
);

// Tests : null embedder → fallback mots-clés interne au router
$router = new SemanticSkillRouter(embedder: new NullEmbeddingProvider());

// Legacy : closure existante qui retourne un seul embedding
$router = new SemanticSkillRouter(
    embedder: fn(string $text): array => $myLegacyEmbedFn($text),
);
// auto-wrappée via CallableEmbeddingProvider — pas de migration
```

### SPI Embeddings — `Memory\Embeddings\*`

Échafaudage de l'item B3 de la roadmap. Contrat d'embedding standardisé utilisé par `SemanticSkillRouter` (et tout futur travail de retrieval / dédup vectoriel).
...
interface EmbeddingProvider
{
    /** @return array<int, array<int, float>> — un vecteur par texte d'entrée */
    public function embed(array $texts): array;
    public function dimensions(): int;
    public function fingerprint(): string;   // clé d'invalidation cache au changement de modèle
}
```

**Implémentations de référence :**

| Classe | Quand l'utiliser | Notes |
|---|---|---|
| `NullEmbeddingProvider` | Tests / environnements de dev | Renvoie `[]` par row ; le downstream traite `[]` comme « aucun signal » et retombe sur le ranker baseline |
| `CallableEmbeddingProvider` | Adapter des closures existantes | Détecte automatiquement la shape batch `fn(array): array` vs la shape legacy mono-texte `fn(string): array<float>` (forme `VectorMemoryProvider`) en inspectant le type du paramètre |
| `OllamaEmbeddingProvider` | Devs qui font déjà tourner Ollama localement | Appelle `/api/embeddings`, modèle par défaut `nomic-embed-text` (768 dims). Une row qui échoue ressort en `[]` — un daemon instable ne désactive pas tout le router pour le reste de la session |
| `OnnxEmbeddingProvider` | Inférence ONNX in-process | Échafaudage opt-in. Le constructeur vérifie le runtime (`ext-onnxruntime` ou le binding userland `ankane/onnxruntime`) et le fichier modèle ; lève une erreur claire « utilisez OllamaEmbeddingProvider en attendant le package compagnon » si l'un manque |

```php
use SuperAgent\Memory\Embeddings\OllamaEmbeddingProvider;

// Le plus ergonomique : daemon Ollama local
$embedder = new OllamaEmbeddingProvider(
    host: 'http://localhost:11434',
    model: 'nomic-embed-text',
);
$vectors = $embedder->embed(['description skill A', 'description skill B']);
// → [[0.123, 0.456, ...], [0.789, 0.012, ...]]
```

`OnnxEmbeddingProvider` expose des hooks de sous-classement (`tokenise()`, `runModel()`) pour les hôtes qui ont déjà un wrapper ONNX, et c'est un type stable — DI / `instanceof` aujourd'hui marchera contre le futur package compagnon `forgeomni/superagent-embeddings`.

### Bridge navigateur (B4)

Contrôle agent par Native Messaging d'un vrai navigateur Firefox / Chromium, sans Selenium / Playwright. Trois classes empilées :

**`Tools\Browser\NativeMessagingTransport`** — emballe le framing JSON préfixé sur 4 octets little-endian que Mozilla / Chromium Native Messaging attend. Détient le cycle de vie du child-process launcher avec lectures bornées par deadline (15 s par défaut), donc une WebExtension bloquée ne deadlocke jamais l'agent. Renvoie null sur EOF / timeout / frame invalide pour que les appelants décident retry / restart / surfacer.

**`Tools\Browser\FirefoxBridge`** — client RPC haut niveau au-dessus du transport. Méthodes :

| Méthode | Usage |
|---|---|
| `navigate($url)` | Pilote la barre d'URL |
| `screenshot()` | Renvoie un PNG en base64 |
| `click($selector)` | Clic sur sélecteur CSS |
| `type($selector, $text)` | Tape dans un input |
| `evalJs($code)` | Exécute du JS dans la page ; renvoie le résultat JSON-sérialisable |
| `wait($selector, $timeoutMs)` | Attend qu'un élément apparaisse |
| `pollEvents()` | Vide les événements non-sollicités buffé par le transport |

Auto-assigne les request ids et buffé les événements non-sollicités pour que les appelants n'aient pas à multiplexer. Le docblock de classe contient le walkthrough setup complet — `manifest.json` minimal pour la WebExtension et le layout du manifest Native Messaging — donc l'intégration est reproductible sans docs externes.

**`Tools\Builtin\FirefoxBridgeTool`** (nom d'outil `browser`) — sous-classe `Tool` qui expose le bridge aux agents. Un seul process launcher par instance d'outil ; la première action le démarre, `action: 'close'` (ou le destructeur) le démonte. Surface de capacités serrée (pas de gestion d'onglets, cookies, ni API d'extension) — on garde le rayon d'explosion d'abus petit.

```php
use SuperAgent\Tools\Builtin\FirefoxBridgeTool;

$agent->registerTool(new FirefoxBridgeTool());
$agent->run('ouvre https://example.com, capture, clique sur "Sign in", recapture');
```

Le chemin du launcher vient de la variable d'env `SUPERAGENT_BROWSER_BRIDGE_PATH` ou du paramètre constructeur `launcherArgv`. Les screenshots reviennent en PNG base64 pour que les hôtes puissent les republier dans leur UI (le `BrowserScreenshotStore` de SuperAICore fait exactement ça pour `/processes`).

### Reprise de session cross-harness

`Conversation\HarnessImporter` — interface pour « charger ce `.jsonl` depuis le store de session sur disque d'un autre harness d'agent et me rendre des `Message[]` internes ». Deux importers de référence :

| Importer | Lit | Notes |
|---|---|---|
| `Conversation\Importers\ClaudeCodeImporter` | `~/.claude/projects/<hash>/<uuid>.jsonl` | Parser line-by-line tolérant qui supporte la dérive de schéma entre versions Claude Code |
| `Conversation\Importers\CodexImporter` | `~/.codex/sessions/**/*.jsonl` | Glob récursif, pour que les anciennes installs Codex (layout différent) marchent toujours |
...
Les messages renvoyés sont la shape interne du SDK (`UserMessage` / `AssistantMessage` / `ToolResultMessage`), prêts à être renvoyés dans un `Agent` — typiquement routés via `Conversation\Transcoder` (0.9.5) pour basculer vers la nouvelle wire family du provider sur lequel l'agent va tourner.
...
// Continuer la même conversation sur Kimi :
...
Commande user-facing pour la reprise cross-harness. Trois sous-commandes :

```bash
# Liste les N dernières sessions Claude Code :
superagent resume list  --from claude

# Affiche le transcript complet d'une session :
superagent resume show  --from claude --session 8e2c-...

# Charge et envoie dans un chat — pipe typique vers la commande chat :
superagent resume load  --from claude --session 8e2c-... \
  | superagent chat --provider kimi --resume-stdin
```

`--from` accepte `claude` / `claude-code` / `cc` / `codex`. `--json` sur `list` émet une row par session pour le scripting. Lit ses propres flags directement depuis `$_SERVER['argv']` (après le token `resume`) — le parser d'options globales n'a pas à apprendre les knobs spécifiques aux importers. Routé dans `SuperAgentApplication`.

`superagent resume` est indépendant du flag `--resume-stdin` de ChatCommand — le pipe est un workflow supporté, mais les hôtes peuvent aussi appeler `ClaudeCodeImporter::load()` directement et alimenter le résultat dans un `Agent` résolu par Laravel (typiquement via `Conversation\Transcoder` pour la continuation cross-wire).

### Notes architecturales

- **L'extraction de symboles d'`agent_grep` est par défaut basée regex** — rapide, sans dépendance, suffisant pour ~95 % des charges d'agent typiques. Les hôtes qui veulent la précision tree-sitter composent `CompositeSymbolExtractor` avec `TreeSitterSymbolExtractor` d'abord ; pas besoin de sous-classer `AgentGrepTool`.
- **`FileLedger`** est dimensionné pour des swarms in-process (dizaines d'agents, centaines de fichiers en vol). Les déploiements plus larges branchent un emitter custom qui forwarde les events vers Redis pub/sub ou des websockets.
- **`AmbientWorker`** est purement une primitive tick-driven — la boucle est décidée par l'hôte. Le worker fait respecter le budget par pass en interne, donc un tick ne bloque jamais plus que quelques secondes.
- **`SemanticSkillRouter`** dégrade gracieusement — sans embedder, il score les skills par chevauchement de mots-clés et le reste du pipeline n'est pas affecté. Le cache vectoriel est keyé par hash du contenu skill, donc renommer/redécrire un skill invalide automatiquement sa row.
...
## 89. Durcissement de la compatibilité Kimi (Moonshot) (v1.0.10)

Trouvé en comparant le chemin Kimi de SuperAgent au client officiel [`kimi-code`](https://github.com/MoonshotAI/kimi-code) de MoonshotAI (`packages/kosong/src/providers/kimi.ts`, `kimi-schema.ts`, `packages/oauth`). Cinq correctifs au niveau du wire — dont deux généralisés à tout provider compatible OpenAI — plus un correctif d'honnêteté sur l'Agent Swarm.

### Normalisation du schéma d'outils (`Format\JsonSchemaNormalizer`)

Le validateur d'outils de Moonshot rejette deux formes extrêmement courantes : les `$ref` / `$defs` locaux (émis par la plupart des schémas codegen et MCP) et les schémas de propriété imbriqués sans `type` (p. ex. les propriétés MCP enum-only). `KimiProvider::convertTools()` passe désormais l'`inputSchema()` de chaque outil par `JsonSchemaNormalizer::normalizeForKimi()` :

```php
use SuperAgent\Format\JsonSchemaNormalizer;

// deref() inline les pointeurs #/$defs, #/definitions et # (anti-cycle, les
// mots-clés frères l'emportent), puis supprime les buckets vidés :
$flat = JsonSchemaNormalizer::deref($schema);

// fillMissingTypes() complète les schémas de propriété sans type : inférence
// par valeur enum/const → par mots-clés structurels → repli 'string' (ne lève jamais) :
$typed = JsonSchemaNormalizer::fillMissingTypes($schema);

// normalizeForKimi() = les deux, dans l'ordre. Le schéma racine est un conteneur
// et reste intact ; seuls les schémas de propriété imbriqués sont complétés.
$wire = JsonSchemaNormalizer::normalizeForKimi($schema);
```

C'est distinct du `Tools\Schema\ProviderNormalizer` opt-in (§84) : celui-ci réécrit un schéma tagué que vous avez rédigé, celui-là s'exécute automatiquement sur le wire Kimi. La passe `deref` est aussi exposée comme hook surchargeable `ChatCompletionsProvider::normalizeToolSchema()` (identité par défaut), pour que d'autres backends stricts — le sous-ensemble OpenAPI de Gemini, le mode strict d'OpenAI — réutilisent le même inlining.

### Généralisé à tout provider compatible OpenAI (`ChatCompletionsProvider`)

- **`stream_options: {include_usage: true}`** est désormais envoyé à chaque requête en streaming (`streamUsageOptions()`, surchargeable, renvoyer `null` pour désactiver). Selon la spec OpenAI — que Moonshot et les autres suivent — une réponse en streaming ne contient **aucun** bloc usage sans lui, mettant silencieusement à zéro le comptage des tokens, des tokens en cache et le suivi des coûts à chaque appel.
- **Le parsing de `usage` en streaming** accepte `choices[0].usage` en plus du `usage` de premier niveau (Moonshot le place là).
- **`completionTokenParam()`** est un point d'extension surchargeable pour le nom du champ de tokens de complétion. `KimiProvider` le surcharge en `max_completion_tokens` : les modèles de raisonnement partagent ce budget avec le canal caché `reasoning_content`, et un petit `max_tokens` produit un HTTP `200` au `content` vide.
- **`extractReasoning()`** (la concaténation des blocs `thinking` en `reasoning_content`) est remontée dans la base et mutualisée ; `DeepSeekProvider` en hérite désormais.

### Round-trip du raisonnement + bascule thinking (`KimiProvider`)

`formatMessages()` rattache le `reasoning_content` précédent du modèle (miroir du replay DeepSeek V4 / série R). `customizeRequestBody()` honore un `reasoning_effort: off` explicite (ou `thinking: false`) en envoyant `thinking: {type: "disabled"}` — le seul moyen de forcer un modèle capable de raisonner à sortir du mode réflexion.

### Découverte des capacités par modèle (`ModelCatalogRefresher`)

`normalize()` mappe les `supports_reasoning` / `supports_image_in` / `supports_video_in` / `supports_tool_use` de Moonshot et les `supported_parameters` d'OpenRouter depuis la réponse `/models` vers la forme catalogue `['thinking'|'vision'|'video'|'tools'|'structured_output' => true]`, pour que `CapabilityRouter` route sur un signal réel par modèle. Attaché uniquement quand la source déclare des capacités ; sinon le repli au niveau provider de `ModelCatalog::capabilitiesFor()` s'applique toujours.

### L'Agent Swarm est opt-in

`kimi-code` ne fournit **aucun** endpoint REST de swarm — son parallélisme vient de sous-agents locaux `coder` / `explore` / `plan`. `KimiSwarmTool` est donc verrouillé derrière `SUPERAGENT_KIMI_SWARM_ENABLED` ; sans lui, l'outil renvoie une erreur explicite au lieu d'appeler un endpoint inexistant. La plomberie au niveau provider `submitSwarm()` / `poll()` / `fetch()` / `cancel()` et ses tests de contrat wire sont conservés pour le jour où Moonshot publiera la spec.

```bash
# Désactivé par défaut ; à activer uniquement vers un endpoint de préversion/privé :
...
## 90. SmartFlow — flux dynamiques cross-modèle (v1.1.0)

`src/SmartFlow/` est un portage PHP du moteur `Workflow` intégré de Claude Code, rendu **cross-modèle / cross-API**. Les mêmes primitives — `agent()`, `parallel()`, `pipeline()`, `gate()`, `budget`, `schema`/`SKIP` — pilotent n'importe lequel des 15 providers. C'est additif : l'ancien `Tools\Builtin\WorkflowTool` est inchangé et rien dans la boucle d'agent ne bouge.

### Moteur + primitives

Un corps de flux est `callable(Flow $flow): mixed`. `FlowEngine::run($flowOrClosure, $args, $opts)` câble un `PersonaRegistry`, un `FlowAgentRunner` (ou `ProcessPool`), un `CallLedger` et un `Budget` dans un contexte `Flow`, exécute le corps et renvoie un `FlowResult`.

```php
use SuperAgent\SmartFlow\{FlowEngine, FlowDefinition, FlowOptions, Flow};

$def = FlowDefinition::make('dev', 'Construire à partir d\'un objectif', function (Flow $flow) {
    $flow->phase('Plan');
    ...
    $reviews = $flow->parallel([                              // concurrent (pool de processus)
        $flow->call('Relire pour la correction', ['role' => 'reviewer']),
        $flow->call('Relire pour la sécurité',   ['role' => 'reviewer', 'provider' => 'openai']),
    ]);

    $drafts = $flow->pipeline(['intro', 'corps'],             // par item, par étape
        fn ($prev, $item) => $flow->call("Rédiger : {$item}", ['role' => 'writer']));

    $gate = $flow->gate('nonempty', fn () => $plan !== $flow->SKIP, [
        'fallback' => fn () => 'plan manuel requis',
    ]);

    return compact('plan', 'reviews', 'drafts', 'gate');
});

$result = (new FlowEngine())->run($def, ['goal' => 'un todo CLI'], new FlowOptions(rehearse: true));
```

| Primitive | Comportement |
|-----------|--------------|
| `agent($prompt, $opts)` | un appel cross-modèle ; avec `schema` renvoie un tableau validé, sinon une chaîne, sinon `$flow->SKIP` |
| `call($prompt, $opts)` | un `AgentCall` **différé** pour `parallel()`/`pipeline()` |
| `parallel([...])` | barrière ; les appels différés partent en concurrence |
| `pipeline($items, ...$stages)` | chaque item à travers chaque étape ; les appels d'une étape sont concurrents |
| `gate($name, $check, $opts)` | point d'acceptation ; `fallback`/`relay`/`required` |
| `council($claim, $lenses)` | juger via N angles, vote majoritaire |
| `log()`, `phase()`, `$flow->budget`, `$flow->args`, `$flow->SKIP`, `$flow->keep($arr)` | état + helpers |

### Filet de sécurité de sortie structurée à 3 couches

`StructuredOutputLadder::resolve($text, $schema, $nativeRequested)` récupère une valeur valide de trois façons et indique la couche gagnante (visible dans `layers` du registre) :

1. **native** — le provider a reçu une demande JSON (`ResponseFormat::jsonSchema()->toOpenAIFormat()`), toute la réponse parse.
2. **submitted** — le modèle a encadré son JSON dans un bloc ```` ```json ````.
3. **extracted** — en dernier recours, un sniff regex du premier `{...}`/`[...]`.

La validation est un `SchemaValidator` sans dépendance (required/type/enum/min·max/longueur/pattern/items/imbriqué). En cas d'échec total, `agent()` renvoie le singleton `Skip` (`$flow->SKIP`) ; retirez null + SKIP d'un lot avec `$flow->keep($values)`.

### Rôles / personas

`PersonaRegistry` fournit 7 personas intégrés (`planner`/`builder`/`reviewer`/`researcher`/`writer`/`critic`/`chair`) plus `resources/flows/personas/personas.yaml` (`pm`, `designer`, `engineer`, `editor`, `translator`, `fundamental-analyst`, …). Surchargez via `config('superagent.smartflow.personas')`. Épinglez un `provider`/`model` par persona pour rendre un flux réellement cross-modèle.

### Budget, gates, registre + reprise

- **Budget** : `--budget-usd` / `config superagent.smartflow.budget.*` / les `defaults` d'un flux. Une fois épuisé, l'appel suivant lève `BudgetExceededException` et le flux échoue proprement.
- **Gates** (`Gate`/`GateResult`) : enregistrés au registre ; une gate `required` qui échoue sans fallback lève `GateFailedException`.
- **Registre + reprise** : chaque run écrit un `CallLedger` JSONL sous `~/.superagent/flows/`. `FlowSignature::forCall()` est content-addressed à partir de l'appel *déclaré* (label + prompt normalisé + schema + role + provider/model épinglés). `--resume <runId>` rejoue le plus long préfixe inchangé depuis le cache (zéro token) et réexécute à partir du premier appel divergent — même mécanisme que `Squad\SquadResumeManager`.

### Vrai parallélisme (pool de processus)

`ProcessPool::runBatch()` dispatche un lot d'`AgentCall` en sous-processus `bin/flow-agent-runner.php` concurrents (`proc_open` + `stream_select`, fallback polling Windows, plafond = `--concurrency` / `config superagent.smartflow.concurrency`). Les appels uniques, les répétitions et les environnements sans `proc_open` retombent en in-process. L'ordre de soumission est préservé ; un worker planté/expiré donne un résultat en échec (→ SKIP/null).

### Répétition à coût nul

`FakeProvider` (clé de registre `fake`, `src/Providers/FakeProvider.php`) renvoie des stubs déterministes conformes au schéma (`SchemaStub`) à zéro token / zéro coût. Activez via `MULTI_AI_FAKE_PROVIDER=1` ou `flow run --rehearse`. Chaque flux livré est garanti de passer la répétition — exercez toute l'orchestration hors ligne.

### Écrire des flux statiques en YAML

`YamlFlowLoader` compile `resources/flows/*.yaml` vers le même moteur. Stratégies d'étape : `solo` (défaut), `parallel` (`agents:`), `pipeline` (`over:` + `stages:`), `gate` (`check:`). Templating : `{{args.x}}`, `{{steps.name.output}}`, chemins pointés, et `{{item}}` dans les étapes de pipeline. `FlowRegistry` découvre les intégrés + `./flows` + `./.superagent/flows`.
...
steps:
  ...
  - name: reviews
    strategy: parallel
    agents:
      - {role: reviewer, prompt: "Relecture :\n{{steps.plan.output}}"}
  - {name: accept, strategy: gate, check: "nonempty:{{steps.plan.output}}", required: true}
return: plan
```

### CLI + les 11 flux intégrés

```bash
superagent flow list                                        # 11 flux
...
Flux livrés : `dev-from-scratch`, `product-trio`, `research-trio`, `code-review-council`, `doc-writer`, `translate-localize`, `mp-article`, `video-creator`, `stock-trio`, `stock-monthly-style`, `stock-veggie`. Les flux de domaine (`stock-*`, `mp-article`, `video-creator`) portent des prompts best-effort signalés dans le fichier ; les flux stock sont à but éducatif uniquement.

La config est sous `config/superagent.php → 'smartflow'` ; les miroirs d'env sont `MULTI_AI_FAKE_PROVIDER` et `SUPERAGENT_FLOW_*`. Guide complet : [docs/smartflow.md](smartflow.md).
...
## 100. Vague de modèles 2026-09 — Fable 5.1 · GPT-6 Astra · Gemini 3.8 Flash · DeepSeek V4.1 Flash · Qwen3.8-Max-0902 · GLM-5.3-Flash (v1.1.12)

Six providers rafraîchis d'un coup, et cette fois trois des nouveaux fleurons changent la **surface de requête**, pas seulement l'id. Le catalogue (`resources/models.json`, `_meta.updated` 2026-09-17), `CostCalculator`, `TokenEstimator`, les seeds de `ModelResolver`, les défauts des providers et le sélecteur `/model` avancent ensemble.

- **Claude Fable 5.1** (`claude-fable-5-1`, 2026-09-01) — nouvelle cible de l'alias `fable` et modèle du palier Squad **EXPERT**. Même tarif que Fable 5 (10 $ / 50 $ par M) avec des **lectures de cache en baisse de 75 %, à 0,25 $/M**. Trois ruptures par rapport à Fable 5, dont une a demandé du code : **l'appel d'outil forcé disparaît** — `tool_choice: {type: "any"}` et `{type: "tool", name: …}` renvoient 400, donc `AnthropicProvider` rétrograde désormais un choix forcé en `auto` (`none` / `auto` passent tels quels, et Fable 5 / Opus / Sonnet gardent l'appel forcé). Les deux autres sont des règles de harness : les blocs de thinking sont liés au modèle qui les a produits, et modifier des tours antérieurs les invalide (preserved thinking — gardez l'historique en append-only). Covered Model : rétention de 30 jours requise, pas de Priority Tier.
- **GPT-6 Astra** (`gpt-6-astra`, 2026-09-03) — nouveau défaut d'`openai-responses` et cible des alias `gpt-6` / `astra`. 1,05 M de contexte / 128 K de sortie, 10 $ / 1 $ en cache / 50 $ par M. Sa molette d'effort va de `low` à `max` et **retire `none`**, présent sur les paliers 5.6 — `normalizeEffortForModel()` gagne donc une branche GPT-6 qui ramène `off`/`none`/`minimal` à `low` (envoyer `none` là-bas est un 400). Deux nouvelles surfaces Responses : les **outils asynchrones** (`async_tools: true` ou une liste de noms → `async: true` sur ces définitions ; Astra continue de raisonner, appelle d'autres outils ou répond aux parties indépendantes pendant l'exécution, et vous renvoyez le résultat plus tard via le `call_id` d'origine) et le **pilotage en cours de tour** via WebSocket. Le flag async est silencieusement retiré avant GPT-6, où il constitue une erreur de validation.
- **Gemini 3.8 Flash** (`gemini-3.8-flash`, GA 2026-09-02) — nouveau défaut `gemini` et cible des alias `gemini` / `gemini-flash-latest`. 1 M de contexte / 64 K de sortie, tarif de lancement 0,75 $ / 0,075 $ en cache / 3,75 $ par M jusqu'au 2026-12-31 (puis 1,50 $/7,50 $ ; le prix de sortie couvre les tokens de thinking). `thinking_level` reste `low|medium|high` et **`MINIMAL` est une erreur de validation ferme** : `GeminiProvider::clampThinkingLevel()` ramène un `minimal` demandé sur `LOW` pour les Flash 3.7+ (la ligne 3.5 reçoit toujours `MINIMAL`). Les paramètres d'échantillonnage sont dépréciés sur ce palier — `temperature` / `top_p` / `top_k` sont désormais retirés au lieu d'être transmis.
- **DeepSeek V4.1 Flash** (`deepseek-flash`, GA 2026-09-10) — nouveau défaut du provider et premier modèle de la nouvelle famille d'architecture DeepSeek : nativement multimodal, 1 M de contexte / 384 K de sortie, base heures creuses 0,15 $ / 0,003 $ cache-hit / 0,60 $ par M (heures pleines 2×). V4 Flash et V4 Flash Vision Exp sont retirés ; `deepseek-v4-flash` y est temporairement routé et retarifé en conséquence. V4 Pro continue d'être servi au-delà de l'arrêt annoncé du 2026-09-14, facturation inchangée.
- **Qwen3.8-Max-0902** (`qwen3.8-max-0902`, 2026-09-02) — nouveau défaut `qwen` / `qwen-anthropic` et cible des alias `qwen` / `qwen-max` ; mêmes 1 M de contexte et 2 $/6 $ par M que la build GA du 0803, avec un codage à l'échelle d'un projet et des agents collaboratifs nettement renforcés. `QwenProvider::isVisionModel()` couvre maintenant toute la ligne `qwen3.8-*`, ce qui fait profiter `qwen3.8-flash` et `qwen3.8-27b` (tous deux catalogués) du flag image HD.
- **GLM-5.3 promu, GLM-5.3-Flash ajouté** — l'API autonome de 5.3 est GA au tarif 5.2 (1,40 $ / 0,26 $ / 4,40 $ par M), donc **`glm-5.3` devient le défaut du provider** et les alias `glm` / `glm5` s'y résolvent ; le repli provisoire au tarif 5.2 disparaît. `glm-5.3-flash` (2026-08-26) est le premier modèle GLM-5 nativement multimodal de Z.ai — 320 B MoE / 18 B actifs, entrée image + vidéo, 1 M de contexte, poids MIT, 0,15 $ / 0,03 $ / 0,50 $ par M. C'est un modèle distinct et non un post-train 5.3 : `isGlm53()` l'exclut et `reasoning_effort: off` y désactive réellement le thinking.

Retarifications sans nouvel id : **Sonnet 5** est à 2 $/10 $ de façon permanente (la hausse à 3 $/15 $ du 2026-09-01 a été annulée), **GPT-5.6** baisse à Sol 4 $/0,40 $/20 $, Terra 2 $/0,20 $/12 $, Luna 0,20 $/0,02 $/1,20 $, et **GPT-5.5** a de nouveau un tarif publié (5 $/0,50 $/30 $). Les agrégateurs suivent : `anthropic/claude-fable-5.1`, `openai/gpt-6-astra`, `google/gemini-3.8-flash`, `deepseek/deepseek-v4.1-flash`, `z-ai/glm-5.3[-flash]`, `moonshotai/kimi-k3`, `minimax/minimax-m3`, `x-ai/grok-build-0.1` et `meta/muse-spark-1.3` sur OpenRouter ; `global.anthropic.claude-fable-5-1` sur Bedrock (l'invocation exige que le mode de rétention du compte soit `aws_review` dans la région) ; Cursor gagne Grok 4.6, Gemini 3.8 Flash, Fable 5.1 et Muse Spark 1.3 en entrées catalogue uniquement.

```php
// Fable 5.1 : un tool_choice forcé est rétrogradé au lieu de renvoyer 400
$agent = new Agent(['provider' => 'anthropic', 'model' => 'claude-fable-5-1']);
$agent->run('extraire les champs', ['tool_choice' => ['type' => 'any']]);
// → le body envoyé porte tool_choice: {type: "auto"}

// GPT-6 Astra : outils asynchrones + une valeur d'effort réellement acceptée
$agent = new Agent(['provider' => 'openai-responses']);      // → gpt-6-astra
$agent->run('migrer le schéma', [
    'async_tools'      => ['run_migration'],                 // → async: true sur cet outil
    'reasoning_effort' => 'off',                             // → reasoning.effort: low (jamais `none`)
]);
```

> **Non livré :** le **Muse Spark 1.3** de Meta (fleuron agentique de codage de MSL, 2026-09-02, 1 M de contexte, 1,25 $/4,25 $ par M) est accessible via OpenRouter et Cursor et y est catalogué, mais SuperAgent n'a pas de provider Meta natif — un provider `meta` natif est un chantier séparé.

Tests : `ModelRefresh202609Test` (rétrogradation et pass-through du tool_choice Fable 5.1, plancher d'effort Astra, gating des outils asynchrones, pins catalogue/tarifs), plus `GeminiProviderTest` (défaut 3.8, clamp MINIMAL), `GlmProviderTest` (défaut 5.3, molette 5.3-Flash), `DeepSeekProviderTest`, `QwenProviderTest`, `OpenAIResponsesProviderTest`, `CostCalculatorTest` mis à jour. Suite complète verte (3375).

## 101. Provider Meta natif — Muse Spark sur la Meta Model API (v1.1.13)

La v1.1.12 avait catalogué Muse Spark mais ne pouvait l'atteindre que via OpenRouter. Cette version ajoute `MetaProvider`, une route native vers la Meta Model API (`https://api.meta.ai`), ce qui rend enfin disponibles les champs de requête propres à Meta.

```php
$agent = new Agent([
    'provider' => 'meta',
    'api_key'  => getenv('META_API_KEY'),   // MODEL_API_KEY (le nom de Meta) est lu aussi
]);                                          // → muse-spark-1.3
```

**Pourquoi une classe de provider plutôt qu'une simple surcharge de base_url.** Muse Spark est *compatible* OpenAI, pas *identique* à OpenAI, et cinq des différences sont des 400 fermes. `MetaProvider` étend `ChatCompletionsProvider` et surcharge exactement ces points :

| Comportement Meta | Où c'est traité |
|---|---|
| `max_completion_tokens`, pas `max_tokens` | `completionTokenParam()` |
| Le raisonnement ne se désactive pas — `reasoning_effort: none` → 400 | `reasoningEffortFragment()` plancher `off`/`none`/`disabled` à `minimal` |
| Le palier `max` n'existe que sur 1.3 | `modelSupportsMaxEffort()` ramène `max` → `xhigh` sur 1.1 / 1.2 |
| `developer` prime sur `system` | `buildRequestBody()` re-rôle le message système hissé |
| `stop` / `logprobs` / `logit_bias` / `prediction` / `modalities` / `audio` / `web_search_options` / `n > 1` → 400 | retirés dans `buildRequestBody()` **après** la fusion d'`extra_body` |

Comme le nettoyage passe en dernier, un corps de requête venu d'un autre provider est assaini au lieu d'être rejeté — la raison habituelle pour laquelle une intégration « il suffit de pointer base_url » casse en production.

**Options spécifiques à Meta.** L'ancrage par recherche est un outil côté serveur et non un drapeau, et Meta le facture à la requête (2,50 $ / 1 000) en plus des tokens :

```php
$agent->run('qu\'est-ce qui a été livré la semaine dernière ?', [
...
`grounding` s'ajoute **à côté** de vos outils de fonction au lieu de les remplacer, et pose `tool_choice` quand la classe de base ne l'a pas fait (elle ne le pose que si des outils de fonction sont présents).

**Trois protocoles, un provider.** Meta sert les mêmes modèles et la même facturation via une Responses API, cette Chat Completions API et une Messages API compatible Anthropic. La route Anthropic ne demande aucun code neuf — la même astuce que la route Anthropic de DeepSeek :
...
**Le palier Contributor se choisit explicitement par id, volontairement.** `muse-spark-1.3-contributor` est le même modèle à 0,10 $ / 0,002 $ / 0,20 $ par M — ~12× moins cher — parce que Meta entraîne ses futurs modèles sur vos prompts et complétions (et vous plafonne à 100 RPM au lieu de 3 000). Il figure au catalogue et dans le sélecteur `/model` avec l'échange explicité, mais la seed de `ModelResolver` ne lui donne **aucun alias** : `muse`, `muse-spark` et `spark` résolvent tous vers le `muse-spark-1.3` standard, donc rien n'est routé implicitement vers un modèle du palier d'entraînement.
