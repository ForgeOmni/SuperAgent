...
/**
 * Token-arity dictionary for popular CLI tools, used by {@see BashCommandClassifier}
 * to identify the "human-understandable command" from a parsed bash command.
 *
 * Each entry maps a prefix string → number of tokens that constitute the command
 * name (flags never count). Longest matching prefix wins. Examples:
 *
 *   git checkout main          → ['git', 'checkout']  (git has arity 2)
 *   docker compose up -d       → ['docker', 'compose'] (docker compose has arity 3 → keep "docker compose")
 *   pnpm run dev               → ['pnpm', 'run']  (pnpm run has arity 3 → keep "pnpm run")
 *   touch foo.txt              → ['touch']  (touch has arity 1)
 *   python script.py           → ['python', 'script.py']  (not in table — default keeps first 2)
 *
 * Dictionary lifted verbatim from opencode's `permission/arity.ts` (110+ commands
 * spanning git, docker, kubernetes, cloud CLIs, package managers, language
 * runtimes, and infra tools). The ARITY table was originally GPT-generated against
 * a strict prompt that's reproduced in opencode's source; porting it unchanged
 * preserves the calibration.
 */
final class BashArity
{
    /**
     * @return list<string> The tokens that form the command name. Never empty
     *                      when $tokens is non-empty.
     */
    public static function prefix(array $tokens): array
    {
        $count = count($tokens);
        for ($len = $count; $len > 0; $len--) {
            $prefix = implode(' ', array_slice($tokens, 0, $len));
            $arity = self::ARITY[$prefix] ?? null;
            if ($arity !== null) {
                return array_slice($tokens, 0, $arity);
            }
        }
        if ($count === 0) {
            return [];
    ...
    /**
     * Convenience: pass a raw command line (already de-piped/de-redirected by the
     * classifier), get back the canonical command label (joined tokens).
     */
    public static function label(string $command): string
    {
        ...
        // Cheap tokenizer — splits on whitespace; the BashCommandClassifier has
        // already done the heavy lifting (pipe/redirect splitting, quoting).
        $tokens = preg_split('/\s+/', $trim) ?: [];
        return implode(' ', self::prefix($tokens));
    }

    /**
     * Lifted from opencode/packages/opencode/src/permission/arity.ts.
     * Alphabetical. Flags never count as tokens.
     *
     * @var array<string, int>
     */
    public const ARITY = [
        // 1-token built-ins / coreutils
        'cat' => 1,        // cat file.txt
        'cd' => 1,         // cd /path/to/dir
