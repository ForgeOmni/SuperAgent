...
[![Version](https://img.shields.io/badge/version-1.2.1-purple)](https://github.com/forgeomni/superagent)
...
| `meta-responses` | Meta Model API — Responses route | Same key/models; `POST /v1/responses` — reasoning replay across turns (encrypted or `previous_response_id`), server-managed state *(v1.1.14)*; background jobs — submit / poll / fetch / cancel / delete *(v1.1.15)* |
...
Meta exposes the same models, and the same billing, over three protocols. All three are reachable:

| Route | Provider | Use it for |
|---|---|---|
| Chat Completions | `meta` | One-shot calls, drop-in OpenAI shape |
| Responses | `meta-responses` | **Agentic loops** — the only route that replays reasoning across turns |
| Messages (Anthropic) | `anthropic` + `base_url` | Claude-shaped clients, no new code |

```php
// Agentic loop — reasoning survives the turn boundary
$agent = new Agent([
    'provider' => 'meta-responses',
    'api_key'  => getenv('META_API_KEY'),
]);
$agent->run('find the bug, fix it, run the tests', [
    'reasoning_effort'  => 'xhigh',
    'reasoning_replay'  => true,    // store:false + include:[reasoning.encrypted_content]
]);
```

The Anthropic route needs no new code either:
...
### Reasoning across turns (`meta-responses`)

On Chat Completions, Muse Spark's chain of thought is discarded at the end of every turn — the next request starts cold. The Responses route carries it, two mutually exclusive ways:

- **Encrypted replay (stateless, recommended).** `reasoning_replay => true` sets `store: false` + `include: ["reasoning.encrypted_content"]`; you resend the conversation each turn and the reasoning rides along as an opaque blob. Nothing is retained server-side.
- **Server-managed state.** `previous_response_id` chains turns and the server rebuilds context from what it stored. Repeated `chat()` calls on one provider instance do this automatically.

Meta rejects a request carrying both, so asking for replay drops the chaining id rather than letting the call 400.

The route also strips the OpenAI-only knobs the shared Responses base can emit — `reasoning.mode`, `reasoning.context`, `text.verbosity`, `service_tier`, `prompt_cache_options`, `response_format` (structured output goes through `text.format`) — and refuses `background: true` outright rather than silently dropping it, since it cannot be combined with streaming and the retrieve/cancel endpoints it implies are not wired.

> `max` effort ("extended reasoning") is **Standard-tier `muse-spark-1.3` only** — 1.1, 1.2 and every `-contributor` id return 400 for it, so the dial degrades them to `xhigh`.

### Background jobs (`meta-responses`)

A turn that runs for many minutes at high effort is the wrong shape for a held-open stream. `background: true` detaches it: the server acknowledges immediately, keeps working, and you come back for the result. `MetaResponsesProvider` implements `SupportsBackgroundResponses` (submit → poll → fetch, plus cancel and delete):
...
// → JobHandle{jobId: "resp_…", kind: "response"} — nothing generated yet
...
$provider->deleteBackground($job);      // stored objects are yours to clean up
```

Or re-attach and render it like a live turn — `followBackground()` streams the stored response (`response.created`, then the terminal event carrying the whole result; no token deltas, since they were produced while nobody was listening):

```php
foreach ($provider->followBackground($job, startingAfter: $lastSeq) as $message) { … }
```

Details the SDK handles for you:

- **Submit forces the detached shape** — `background: true`, `stream: false` (the pair is a 400) and `store: true`. Without storage Meta drops a background response after ~10 minutes, which would race your polling. That also rules out `reasoning_replay`, which is the stateless mode; a background job is server-side state by definition.
- **`incomplete` is not a failure.** A job that hit `max_output_tokens` is terminal *with* output: `poll()` reports `Done` and `fetch()` returns the truncated content with `StopReason::MaxTokens`. Only `failed` throws.
- **A cancel that loses the race still counts.** If the turn completes in the same instant, Meta returns the completed response rather than an error; `cancel()` reports true for any terminal state, since either way you stop polling.
- **`deleteBackground()` returns false when the object is already gone** (Meta answers 404 — the endpoint is not idempotent).
- Passing `background: true` to a normal `chat()` raises with a message pointing at `submitBackground()`, rather than dropping the flag and streaming anyway.

`countInputTokens()` wraps `POST /v1/responses/input_tokens` — ask whether a trimmed conversation fits before spending a turn finding out.

Muse Spark is also reachable through OpenRouter (`meta/muse-spark-1.3`) and Cursor; the native providers are the only routes that speak these Meta-specific fields.

*Since v1.1.13; Responses route since v1.1.14*

---

## MiniMax M3

MiniMax M3 (released 2026-06-01, the `minimax` default) is the **MSA-architecture** flagship: a **1M-token context** (512K max output), **native multimodality** (image *and* video input trained from step 0), and a single-model **interleaved-thinking toggle**. Standard pay-as-you-go pricing is **$0.60 in / $2.40 out** per million tokens (a 7-day launch promo currently halves it to $0.30/$1.20; image/video input billed at $1.00/M) — thinking and non-thinking share one price. `MiniMax-M2.7` stays available by id or the `m2` alias.
...
`adaptive` (the model picks depth per turn) is MiniMax's recommended default; `disabled` is the low-latency path for chat / code completion. Reasoning streams back as a separate `ContentBlock::thinking()` block — the same channel DeepSeek V4 uses, so the rendering code is shared.

**Native multimodality.** Image and video ride the standard OpenAI-style content parts — `image_url` and `video_url` — in your message content; no special flags.
...
`gemini-3.5-pro` / `gemini-3.5-flash` / `gemini-3.5-flash-lite` are first-class catalog entries (alias `gemini` → 3.5 Pro). The 3.x preview SKUs from gemini-cli (`gemini-3-pro-preview`, `gemini-3.1-pro-preview`, `gemini-3-flash-preview`, `gemini-3.1-flash-lite-preview`) ship too. Provider default upgraded from `gemini-2.0-flash` → `gemini-3.5-flash`.
...
`Guardrails\LoopDetector` already catches hash-identical repetition (5 tool-call dupes, 10-char content chants, 8 file-reads in a 15-call window, …). New `Guardrails\LlmLoopChecker` complements it with a Flash-model probe after the 30th turn — catches *semantic* loops the hashes miss (model paraphrases the same plan ten times without acting). Prompt is the verbatim gemini-cli `LOOP_DETECTION_SYSTEM_PROMPT`; check interval adjusts dynamically (5–15 turns).
...
| Module                              | What it does                                                                                                                                                                                                                  |
|-------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Permissions\BashArity`             | 110+ CLI arity table (git→2, `docker compose`→3, `terraform workspace`→3, `vault kv`→3, …). `BashCommandClassifier::extractPrefix()` uses longest-prefix-wins so permission rules match the right granularity.                |
| `Context\Strategies\ConversationCompressor::getStructuredSummaryPrompt()` | 7-section Markdown template (Goal / Constraints / Progress·Done·InProgress·Blocked / Decisions / Next Steps / Critical Context / Relevant Files). Select via `options: ['summary_prompt' => 'structured']`. |
| `Format\` namespace                 | 26 auto-formatters (gofmt, prettier, biome, ruff, rustfmt, **pint**, rubocop, shfmt, clang-format, terraform fmt, …). `(new FormatterRunner())->formatFile($path, $worktree)` runs every applicable formatter after an edit. |
