...
**Effort de raisonnement.** GLM-5.2 ajoute une molette `reasoning_effort` par-dessus le bascule binaire `thinking` ; `GlmProvider` implémente à la fois `SupportsThinking` et `SupportsReasoningEffort`. Trois façons de le piloter :

```php
// 1. Bascule thinking directe — true | 'enabled' | 'disabled'
$agent->run('hard reasoning prompt', ['thinking' => true]);

// 2. Molette reasoning-effort — off → disabled, low…high → high, max → max
...
`off` envoie `thinking: {type: disabled}` ; `high` et `max` envoient `reasoning_effort` accompagné de `thinking: {type: enabled}` (`reasoning_effort` l'emporte sur un simple bascule `thinking` lorsque les deux sont définis). Le raisonnement revient dans un bloc `ContentBlock::thinking()` séparé via le canal partagé `delta.reasoning_content` — le même qu'utilise DeepSeek V4.

**Outils compagnons.** Les outils côté serveur de GLM restent disponibles : `glm_web_search`, `glm_web_reader`, `glm_ocr`, `glm_asr`.
...
fichiers, git et HTTP se chargent sauf indication contraire.

`embedded` est le profil pour faire tourner le SDK à l'intérieur de votre
propre produit, pour des personnes qui n'en sont pas les développeurs. Rien ne
se charge que vous n'ayez remis vous-même, et tout ce qui peut atteindre la
machine ou le réseau est refusé même si c'est remis par erreur.
...
$agent = Agent::embedded([
    'provider' => 'anthropic',
    'tools'    => $myDomainTools,   // les vôtres, et eux seuls
]);
```

Un profil ne fournit que des **valeurs par défaut** — ce que vous passez
l'emporte, dans les deux sens. Définissez `SUPERAGENT_PROFILE=embedded`, ou
`superagent.profile`, pour en faire le défaut de tout le processus.

### Politique d'outils

`allowed_tools` / `denied_tools` filtrent les appels par **nom**. Une politique
filtre par ce qu'un outil **est** : sa catégorie, et s'il se contente de lire.
...
$agent = new Agent([
    ...
    'tool_policy' => [
        ...
        'read_only_only'  => true,     // refuser tout ce qui écrit
        'allow_list'      => null,     // ou une liste de noms
        'deny_list'       => [],
    ],
]);
```

Elle est appliquée **deux fois** : à l'assemblage de la liste d'outils, puis de
nouveau juste avant chaque appel. C'est le second contrôle qui compte — un
outil peut arriver après la construction de l'agent (un plugin, le catalogue
d'un serveur MCP, un outil intégré ajouté par une mise à jour du SDK), et « cet
agent ne peut pas exécuter de commandes shell » doit être une propriété de
l'agent, pas une conséquence du soin apporté à ses arguments de construction.

Les outils que vous nommez vous-même et que la politique refuse lèvent
`ToolPolicyException` à la construction : une contradiction dans votre propre
configuration, signalée au plus tôt. Ceux produits par le chargeur sont
simplement filtrés. Un appel refusé revient au modèle sous forme de résultat
d'erreur nommant la règle, donc la conversation continue.

`embedded` applique `ToolPolicy::HOST_CATEGORIES` par défaut. Passez
`'tool_policy' => false` pour y renoncer, ou votre propre spécification pour
...
Un portage PHP du moteur `Workflow` de Claude Code, rendu **cross-modèle /
cross-API** : les mêmes primitives — `agent()`, `parallel()`, `pipeline()`,
`gate()`, `budget`, `schema`/`SKIP` — pilotent n'importe lequel des 15 providers.
*Un seul jeu de primitives, plusieurs cerveaux.*

```bash
superagent flow list                                            # 11 flux intégrés
superagent flow run dev-from-scratch --args goal="todo CLI" --rehearse   # répétition à 0 $
superagent flow run product-trio --args idea="un tracker d'habitudes"   # réel, cross-modèle
superagent flow run research-trio --args question="…" --resume <run-id>  # préfixe en cache
```

Au-delà du moteur intégré, il ajoute : un **filet de sécurité de sortie
structurée à 3 couches** (native → soumise → extraite, avec un sentinel `SKIP`
au lieu d'un crash), des **rôles / personas** réutilisables, des **gates** avec
acceptation fallback/relay, un **registre d'appels content-addressed** rendant
les runs **reprenables sans redépenser de tokens**, un **vrai parallélisme par
pool de processus**, et un mode **répétition à coût nul `MULTI_AI_FAKE_PROVIDER=1`**
(chaque flux livré est garanti de passer la répétition).

```php
use SuperAgent\SmartFlow\{FlowEngine, FlowDefinition, FlowOptions, Flow};

$def = FlowDefinition::make('review', 'Relire un diff', function (Flow $flow) {
    $reviews = $flow->parallel([
        $flow->call('Relire pour la correction : ' . $flow->args['diff'], ['role' => 'reviewer']),
        $flow->call('Relire pour la sécurité : '   . $flow->args['diff'], ['role' => 'reviewer', 'provider' => 'openai']),
    ]);
    $verdict = $flow->agent('Consolider : ' . json_encode($reviews), [
        'role' => 'chair',
        'schema' => ['type' => 'object', 'required' => ['decision'],
                     'properties' => ['decision' => ['type' => 'string', 'enum' => ['approve', 'request_changes']]]],
    ]);
    return $verdict;   // tableau validé, ou $flow->SKIP en cas d'échec du schéma
});

$result = (new FlowEngine())->run($def, ['diff' => $diff], new FlowOptions(rehearse: true));
```

Les flux statiques s'écrivent aussi en YAML dans `resources/flows/*.yaml`
(stratégies : `solo` / `parallel` / `pipeline` / `gate`, templating
`{{args.x}}` / `{{steps.name.output}}`). Guide complet : **`docs/smartflow.md`**.
