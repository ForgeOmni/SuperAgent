...
- **Qwen3.8-Max** (`qwen3.8-max`, GA 2026-08-03 — successor to the 3.8-Max preview) — Alibaba's multimodal reasoning flagship (2.4T-param MoE, ~95B active): 1M ctx / 131K max output, text+vision input, tool calling + structured outputs; now the `qwen` / `qwen-anthropic` provider default and the `qwen` / `qwen-max` alias target ($2 in / $6 out per M — down from 3.7-Max's $2.50/$7.50). `QwenProvider::isVisionModel()` classes it vision-capable so `vl_high_resolution_images` rides along; `qwen3.7-max` stays reachable (alias `qwen3.7`) as the previous text-only flagship.
- **Gemini 3.7 Flash** (`gemini-3.7-flash`, GA 2026-08-13) — Google's coding/agent flagship and recommended migration target from 3.5 Flash / 3 Flash / 3.1 Pro; now the `gemini` provider default and the `gemini` alias target. 1M ctx / 64K output, `thinking_level` dial `low|medium|high` (default `medium` — the `minimal` tier is gone; `temperature`/`top_p`/`top_k`/`thinking_budget` deprecated on this tier). Intro pricing $0.75 in / $0.075 cached / $3.75 out per M through 2026-12-31 (standard $1.50/$7.50 from 2027-01-01). `gemini-3.5-flash` stays reachable as previous flagship.
- **GLM-5.3** (`glm-5.3`, released 2026-08-14; 1M-context route `glm-5.3[1m]`, alias `glm5.3`) — Z.ai's coding + cyber-defense post-train of the 5.2 base. Widens the effort dial to a genuine `low|high|max` (server default `max`) and makes thinking mandatory: `GlmProvider::reasoningEffortFragment()` on 5.3 ids maps `off`→`low` (thinking cannot be disabled, matching Z.ai's Coding Plan adapters) and keeps `low` as its own tier instead of collapsing onto `high`. Standalone API is in staged rollout with per-token pricing unpublished, so `glm-5.2` remains the provider default; cost tracking provisionally uses the 5.2 rate ($1.40/$4.40) via CostCalculator's static table. Open weights promised ~2 weeks post-launch.
...
**GPT-5.6 (Sol / Terra / Luna) and Grok 4.5 land with their new request surfaces, and a fleet-wide catalog refresh corrects Gemini / DeepSeek / MiniMax / GLM / Qwen pricing and adds Gemini 3.5 Flash + Kimi K2.7 Code.** GPT-5.6 (GA 2026-07-09) replaces GPT-5.5 and retires the mini/nano naming — `gpt-5.6-sol` ($5/$0.50 cached/$30 per M) / `gpt-5.6-terra` ($2.50/$0.25/$15) / `gpt-5.6-luna` ($1/$0.10/$6), all 1.05M context / 128K output with vision. `OpenAIResponsesProvider` now defaults to `gpt-5.6-sol` and speaks the 5.6 surface: the effort dial gains `none`/`max` (with `minimal` retired — values are normalized per generation), plus first-class `reasoning_mode` (`standard`/`pro` — Sol Pro), `reasoning_context` (`auto`/`all_turns`/`current_turn`) and `prompt_cache_options` (explicit caching). Grok 4.5 (released 2026-07-08, $2/$0.50 cached/$6 per M, 500K ctx) becomes the `grok` default with its always-on three-level `reasoning_effort` dial and `x-grok-conv-id` cache pinning. On the Gemini side, the catalog is corrected to reality: `gemini-3.5-pro` / `gemini-3.5-flash-lite` never shipped and are removed, `gemini-3.5-flash` (the actual flagship, $1.50/$9) + `gemini-3.1-flash-lite` land at official prices, the retired 2.0 line is de-defaulted, and `GeminiProvider` gains the 3.5-generation `thinking_level` control + `reasoning_effort` mapping. Additive and non-breaking on top of 1.1.5.

No breaking changes to explicit configs — every previously shipped id stays reachable. Zero-config defaults move: `openai-responses` `gpt-5` → `gpt-5.6-sol`, `grok` `grok-4.3` → `grok-4.5`, `gemini` `gemini-2.0-flash` (retired upstream 2026-06-01) → `gemini-3.5-flash`; the `grok` / `gemini` / `gemini-pro` aliases resolve to the new flagships.

### Added

- **GPT-5.6 series** in the catalog (`resources/models.json`, `openai` + `openrouter` mirrors) — `gpt-5.6-sol` (aliases `gpt-5.6` / `gpt5.6` / `sol`), `gpt-5.6-terra` (`terra`), `gpt-5.6-luna` (`luna`); 1.05M ctx / 128K output, vision, thinking, cached-input prices recorded via `cache_hit_input`. Long-context surcharge (2× in / 1.5× out beyond 272K input) noted per entry.
- **GPT-5.6 request surface** on `OpenAIResponsesProvider` — `reasoning_mode` → `reasoning.mode` (`standard`|`pro`, invalid values dropped), `reasoning_context` → `reasoning.context` (`auto`|`all_turns`|`current_turn`), `prompt_cache_options` → explicit prompt caching (writes 1.25×, reads keep −90%), and the flat cross-provider `reasoning_effort` option. The provider now implements `SupportsReasoningEffort`; `normalizeEffortForModel()` keeps effort legal per generation (`gpt-5.6*`: `minimal`→`low`, `off`→`none`, `highest`→`max`; pre-5.6: `max`/`highest`→`xhigh`, `none`/`off`→`minimal`), and unknown values pass through so server-side additions keep working.
- **Grok 4.5** in the catalog (+ `x-ai/grok-4.5` openrouter mirror) and promoted to the `grok` default (`GrokProvider::defaultModel()` + `ProviderRegistry`) — $2 in / $0.50 cached / $6 out per M (2× beyond 200K prompt), 500K ctx, vision + server-side tools + remote MCP; `grok` / `grok-4.5` / `grok-4.5-latest` aliases.
- **Grok 4.5 reasoning dial + cache pinning** — `GrokProvider::reasoningEffortFragment()` emits the three-level `low`|`medium`|`high` dial for `grok-4.5*` (`max`/`xhigh` clamp to `high`; `off` sends nothing — 4.5's reasoning can't be disabled) while keeping the two-level mini mapping and the empty fragment for grok-4.3/4/3; `conversation_id` (or `prompt_cache_key`) in the provider config is sent as the `x-grok-conv-id` header for xAI's recommended cache routing.
- **Gemini 3.5-generation thinking controls** — `GeminiProvider` accepts `thinking_level` (`minimal`|`low`|`medium`|`high`, the control that replaces `thinkingBudget` — exactly one is emitted since mixing 400s) and maps the cross-provider `reasoning_effort` dial onto it (`off` suppresses `thinkingConfig`; `max`/`xhigh` → `HIGH`).
- **`gemini-3.1-flash-lite`** (GA 2026-05-07, $0.25/$1.50 per M) and **Kimi K2.7 Code** (`kimi-k2.7-code` 2026-06-12 coding flagship, $0.95/$0.19 cache-hit/$4 per M, 262K ctx / 32K out, thinking forced on, image+video input; plus `kimi-k2.7-code-highspeed` at 2×) in the catalog.
- Surfaced the new models in the `/model` picker fallbacks (`CommandRouter` — new `grok` branch + `grok` model-prefix inference, refreshed `openai` / `gemini` lists) and priced GPT-5.6 / GPT-5 / Grok 4.5 in the `CostCalculator` static fallback table. `TokenEstimator` context windows added: `gpt-5.6` → 1.05M, `gpt-5` → 400K, `grok-4.5` → 500K, `grok-4.3` → 1M, `grok-4-fast` → 2M.
- Tests: GPT-5.6 surface cases in `Providers\OpenAIResponsesProviderTest` (default model, per-generation effort normalization, flat dial + fragment, mode/context knobs, `prompt_cache_options`), Grok 4.5 cases in `Providers\GrokProviderTest` (default, 500K capability, pricing, alias, three-level dial, conv-id header), Gemini `thinking_level` / effort cases in `Providers\GeminiProviderTest`, and GPT-5.6/Grok-4.5 pricing in `CostCalculatorTest`.

### Changed

- **Zero-config defaults** — `openai-responses` → `gpt-5.6-sol`, `grok` → `grok-4.5` (provider `defaultModel()` + `ProviderRegistry`), `gemini` → `gemini-3.5-flash` (the 2.0 line was retired upstream 2026-06-01). `ProviderRegistry::getCapabilities('grok')` max_context now reflects the 4.5 flagship (500K).
- **Gemini catalog corrected to reality** — removed `gemini-3.5-pro` and `gemini-3.5-flash-lite` (never publicly shipped; 3.5 Pro is still limited-preview with no API id); `gemini-3.5-flash` repriced to the official **$1.50 / $0.15 cached / $9** per M (was a fictitious $0.40/$3) and promoted to flagship aliases (`gemini` / `gemini-3.5` / `gemini-flash-latest`); `gemini-3.1-pro-preview` output corrected **$15 → $12** per M (≤200K tier; $4/$18 above) and takes the `gemini-pro` alias; retired ids (`gemini-3-pro-preview`, `gemini-3.1-flash-lite-preview`, the 2.0 line) marked with `replaced_by`. `ModelResolver` Gemini seeds updated so the bare `gemini` alias lands on a live model even without a catalog.
- **Fleet-wide pricing corrections** (official vendor pages, 2026-07-12): DeepSeek `deepseek-v4-flash` output **$0.55 → $0.28** per M (+ `cache_hit_input` $0.0028, 384K max output; V4 GA mid-July brings peak-hour 2× pricing — current prices become the off-peak base); MiniMax M3 to its permanent-50%-off tiered price **$0.30/$1.20** per M ≤512K input (cache-read $0.06; $0.60/$2.40 above; priority tier 1.5×); Qwen `qwen3.7-plus` to the GA tiered price **$0.40/$1.60** per M ≤256K (multimodal image+video); GLM `glm-5-turbo` / `glm-5v-turbo` to the official **$1.20/$4** per M. Anthropic verified current — no changes. `CostCalculatorTest` deepseek-v4-flash expectations updated.
- **`OpenAIProvider`** legacy `getSupportedModels()` refreshed with the GPT-5/5.6 line; `supportsStructuredOutput()` now covers all `gpt-5*` ids.
- READMEs (EN / 中文 / FR) gain **GPT-5.6** and **Grok 4.5** sections + refreshed provider-table rows; `docs/ADVANCED_USAGE.*` add §93 (GPT-5.6 surface), §94 (Grok 4.5) and §95 (catalog refresh); `INSTALL.*` version examples bumped to v1.1.6; `SuperAgentApplication::VERSION` aligned to the release (was stale at 1.1.1).

## [1.1.5] - 2026-07-03

### 💻 Summary

**Fable 5 (`claude-fable-5`) and Sonnet 5 (`claude-sonnet-5`) land as first-class `anthropic` models, and `AnthropicProvider` gains the adaptive-only request surface plus an effort dial.** Fable 5 is Anthropic's most capable model — 1M context (128K max output), high-res vision, and an always-on adaptive-thinking surface that differs from the Opus tier in three ways the SDK now handles automatically: it emits `thinking: {type: "adaptive"}` (never `budget_tokens`), it drops `temperature`/`top_p`/`top_k` and trailing assistant prefills, and it exposes Anthropic's GA `output_config.effort` dial via `SupportsReasoningEffort`. **Sonnet 5** ships alongside as the new `sonnet` flagship on the same Claude-5-generation adaptive surface, at the Sonnet price tier. Fable 5 is promoted to the Squad **EXPERT** tier; the zero-config `anthropic` default is set to **Claude Opus 4.8** (from the retired `claude-3-5-sonnet-20241022`). Catalog pricing uses Anthropic's official rates — Fable 5 **$10/$50**, Opus 4.8 **$5/$25**, Sonnet 5 **$3/$15** (intro **$2/$10** through 2026-08-31), Haiku 4.5 **$1/$5** — and this release also corrects the previously stale current-Opus ($15/$75 → $5/$25) and Haiku 4.5 ($0.80/$4 → $1/$5) pricing. Additive and non-breaking on top of 1.1.2 — every existing Claude id keeps working, and the same guards fix the latent 400s that Opus 4.7/4.8 already hit on `budget_tokens`/sampling params.

No breaking changes. The `fable` / `claude-fable` / `claude-fable-5` aliases resolve to Fable 5 (own family, no collision with `opus`); `sonnet` / `claude-sonnet` / `sonnet-5` now resolve to Sonnet 5 (newest in the `sonnet` family).

### Added

- **Fable 5** in the catalog (`resources/models.json`, `anthropic` + `openrouter` + `bedrock` blocks) — Anthropic's most capable model: 1M context / 128K max output, high-res vision, always-on adaptive thinking, effort dial, MCP. Aliases `fable` / `claude-fable` / `claude-fable-5`. Official pricing **$10 in / $50 out** per M (native, above the Opus tier), `anthropic/claude-fable-5` and `anthropic.claude-fable-5-v1:0` mirrored. Refusals fall back to Opus 4.8.
- **Sonnet 5** in the catalog — the most agentic Sonnet yet (released 2026-06-30), close to Opus 4.8 at a lower price. 1M context / 128K max output, adaptive thinking + effort dial, at the Sonnet **$3 in / $15 out** per M tier (intro **$2/$10** through 2026-08-31). Registered as the newest entry of the `sonnet` family (`ModelResolver` seed + `resources/models.json`), so `sonnet` / `claude-sonnet` / `sonnet-5` / `claude-sonnet-5` resolve to it; `openrouter` and `bedrock` ids mirrored. Shares Fable 5's adaptive-only request guards.
- **Anthropic reasoning-effort dial** — `AnthropicProvider` now implements `SupportsReasoningEffort`, mapping `$options['reasoning_effort']` → `output_config.effort` (`low`/`medium`/`high`/`xhigh`/`max`) for Fable 5, Sonnet 5, Opus 4.5+, and Sonnet 4.6. Unsupported models and `off`/unknown values yield no `output_config`, so a stray effort never 400s a request.
- Surfaced `claude-fable-5` and `claude-sonnet-5` in the interactive `/model` picker for `anthropic` (`CommandRouter`, with `claude-opus-4-8` kept as the top/default pick) and priced them in the `CostCalculator` static fallback table.
- Tests: `Providers\AnthropicProviderTest` (adaptive thinking, dropped sampling/prefill, effort mapping, model-aware `thinkingRequestFragment`), `Thinking\ThinkingConfigTest` (adaptive-only detection + `{type: adaptive}` emission, incl. Sonnet 5), and Fable/Opus pricing cases in `CostCalculatorTest`.

### Changed

- **`anthropic` default → Opus 4.8; Squad EXPERT tier → Fable 5** — `ProviderRegistry` default config and the `AnthropicProvider` constructor fallback now resolve zero-config `anthropic` to `claude-opus-4-8` (was the retired `claude-3-5-sonnet-20241022` / `claude-sonnet-4-20250514`), and `Squad\ModelTierMap` routes the **EXPERT** difficulty band to `claude-fable-5` (was `claude-opus-4-7`). Every id remains reachable by explicit config.
- **`sonnet` family flagship → Sonnet 5** — the `ModelResolver` sonnet seed adds `claude-sonnet-5` as the newest entry, so `sonnet` (and every `sonnet`-ish alias) resolves to `claude-sonnet-5` instead of `claude-sonnet-4-20250514`. `ModelResolverTest::test_resolves_sonnet_alias` updated accordingly.
- **Corrected stale Anthropic pricing to official rates** — the current Opus line (`claude-opus-4-5` / `4-6` / `4-7` / `4-8`) is repriced from the outdated $15/$75 to Anthropic's official **$5/$25** per M, and **Haiku 4.5** from $0.80/$4.0 to the official **$1/$5**, across `resources/models.json` (native + openrouter + bedrock) and the `CostCalculator` fallback table. `claude-opus-4-1` and the dated Opus 4.0 / Opus 3 snapshots keep their historical $15/$75. Tests updated: `WorkflowToolDynamicTest::test_opus_4_8_pricing_is_registered` ($90 → $30), `CostCalculatorTest::test_fable_pricing` ($180 → $60), and `ModelCatalogTest` Opus 4.7 lookups ($15/$75 → $5/$25).
- **`ThinkingConfig` adaptive surface expanded and corrected** — `modelSupportsAdaptiveThinking()` now covers Opus 4.6/4.7/4.8, Sonnet 4.6, Fable 5, and Sonnet 5, and `toApiParameter()` emits the bare `{type: "adaptive"}` for them (no `budget_tokens`). This fixes the request shape for Fable 5 / Sonnet 5 (which 400 on `budget_tokens`) and the latent same-cause 400 on Opus 4.7/4.8; older models keep fixed-budget `{type: "enabled", budget_tokens}`.
- **`AnthropicProvider::buildRequestBody()` guards the adaptive-only family** — drops `temperature`/`top_p`/`top_k` for Fable 5 / Sonnet 5 / Opus 4.7 / 4.8, skips trailing assistant prefills for the 4.6+ family and Fable 5 / Sonnet 5, and routes the `thinking_budget_tokens` shorthand and the `SupportsThinking` fragment through the adaptive shape on those models.
- **`TokenEstimator` context windows** — added `claude-fable` and `claude-sonnet-5` → 1M, plus `claude-opus-4` / `claude-sonnet-4` → 1M so the 4.x ids match their real 1M window (previously fell back to the 200K default), preventing premature compaction.
- READMEs (EN / 中文 / FR) gain a **Fable 5 & Sonnet 5** section (adaptive surface, effort dial, 30-day-retention caveat) and an updated `anthropic` provider-table row; `docs/ADVANCED_USAGE.*` document the adaptive-only surface and effort dial (new `## 92` section); `INSTALL.*` version examples bumped to v1.1.5.
...
**DeepSeek V4 provider + cross-provider reasoning channel + cache-aware billing.** First-class `deepseek` provider lands as an OpenAI-wire `ChatCompletionsProvider` subclass covering V4-Pro (1.6T MoE / 49B active) and V4-Flash (284B / 13B active), both with 1M context and `thinking: {type: enabled}` toggle. Same V4 endpoint also exposes an Anthropic-compatible mode at `/anthropic` — `AnthropicProvider` already accepts a custom `base_url`, so the same SDK shells over either wire family. The shared `ChatCompletionsProvider` SSE parser learns to surface vendor `delta.reasoning_content` as a separate `ContentBlock::thinking()` block, so DeepSeek-V4-thinking, R1, Kimi-thinking, Qwen-reasoning and any future OpenAI-compat reasoner stream their internal monologue on a distinct channel that callers can render or hide deliberately. Cache hit/miss billing gets a third accepted shape (`prompt_cache_hit_tokens`, DeepSeek's historical V3 wire) plus an OpenAI-side rebalance: `prompt_tokens` is gross on every OpenAI-compat backend, so the parser now subtracts the cached portion before populating `Usage::inputTokens` — Kimi, OpenAI, and DeepSeek were silently being billed at ~110% of true cost when caching was in play.

**Deprecation lane.** `models.json` schema gains optional `deprecated_until` (ISO date) and `replaced_by` (canonical id) fields on model rows. `deepseek-chat` and `deepseek-reasoner` are flagged with `deprecated_until: 2026-07-24, replaced_by: deepseek-v4-flash | deepseek-v4-pro` per DeepSeek's announced retirement. `ModelResolver::resolve()` emits a one-shot `error_log` warning per `(model, process)` pair when a deprecated id resolves, so users on the legacy aliases get an actionable deadline instead of a surprise 404 at the end of July. `SUPERAGENT_SUPPRESS_DEPRECATION=1` silences the warning for CI / scripted contexts that pin to a deprecated id deliberately. The same mechanism will cover future provider retirements without per-vendor code.

**Rationale.** DeepSeek published V4 on 2026-04-24 with a hard retirement timeline for V3 / R1; integrating the new ids before the deprecation window closes lets downstream apps roll over on their own schedule rather than on DeepSeek's clock. The `reasoning_content` plumbing was due regardless — Qwen's `qwen-reasoning` and Kimi's thinking-tuned variants emit the same field, and surfacing it generically in the base parser means every existing OpenAI-compat subclass gains the capability without an override. The cache-billing fix is the costliest correctness win in this release: a Kimi or OpenAI conversation with a 90% cache hit rate was being billed at full price for the cached tokens *plus* an additive 10% surcharge, instead of just the 10% read price — a 10× error on the cached slice.

**Zero public method signatures broke.** New `DeepSeekProvider` is purely additive. `ChatCompletionsProvider::parseSSEStream()` still yields a single `AssistantMessage`; the only observable shape change is an extra leading `ContentBlock` of type `thinking` when the upstream actually emits reasoning text (no thinking text → no extra block, byte-identical to 0.9.5). `Usage::inputTokens` now excludes the cached portion on OpenAI-compat backends — this is a behaviour change but the only direction it moves cost numbers is *down* (toward correctness); `Usage::totalTokens()` keeps adding the fields back so total counting is unchanged. Anthropic's path was already wire-correct and is unaffected.

### Added

#### DeepSeek provider
- **`SuperAgent\Providers\DeepSeekProvider`** — OpenAI-wire `ChatCompletionsProvider` subclass, ~80 lines. Implements `SupportsThinking` (returns `thinking: {type: enabled}` — V4 controls the budget server-side based on tier). Two regions: `default` → `https://api.deepseek.com`, `beta` → `https://api.deepseek.com/beta` for FIM / prefix completions. Default model `deepseek-v4-flash` (matches the price/quality V3 users already paid for; V4 currently routes the retired `deepseek-chat` alias here). `customizeRequestBody()` accepts `$options['thinking'] = true | ['type' => 'enabled', ...]` as a top-level knob alongside the generic `$options['features']['thinking']` path.
- **`ProviderRegistry::register('deepseek', DeepSeekProvider::class)`** — registered with default config (`model: deepseek-v4-flash`, `region: default`, `max_tokens: 8192`, `max_retries: 3`), env discovery (`DEEPSEEK_API_KEY` + optional `DEEPSEEK_REGION`), health probe (`GET /v1/models`), capabilities map (`max_context: 1_048_576`, `thinking: true`, `regions: ['default', 'beta']`). `validateConfig()` requires `api_key` like the other API-key providers.
- **`AnthropicProvider` docblock** — documents the `base_url` override pattern for DeepSeek's Anthropic-compat endpoint at `https://api.deepseek.com/anthropic`. No code change in `AnthropicProvider` was needed; the existing `$config['base_url']` plumbing already supports it. Use the OpenAI-compat `DeepSeekProvider` when you want the OpenAI wire shape; configure `provider=anthropic, base_url=https://api.deepseek.com/anthropic, api_key=$DEEPSEEK_API_KEY, model=deepseek-v4-pro` when you want the Anthropic shape (e.g. to keep an Anthropic-prompted history wire-identical across providers).

#### Reasoning content channel
- **`ChatCompletionsProvider::parseSSEStream()`** now accumulates `delta.reasoning_content` on a separate buffer alongside `delta.content`. End-of-stream the accumulated reasoning is emitted as a single `ContentBlock::thinking($reasoningContent)` prepended to the assistant's content list (so renderers iterating in order see reasoning-then-answer, matching Anthropic's native `thinking + text` ordering). Streaming handlers' `onText` callback continues to fire only for the user-facing text channel — reasoning is out-of-band by design so callers can render or hide it deliberately. No-op when the upstream never emits the field, so every OpenAI-compat subclass that doesn't have a reasoning mode is byte-identical to 0.9.5.

#### Catalog deprecation metadata
- **`ModelCatalog::deprecation(string $id): ?array{deprecated_until: string, replaced_by: ?string, days_left: int}`** — reads optional `deprecated_until` (ISO `YYYY-MM-DD`) and `replaced_by` fields off the catalog entry. Returns `null` for current models. `days_left` is negative once the deprecation window has lapsed; callers use it to gate "you have N days" warnings.
- **`ModelResolver::resolve()`** — wraps the existing pure resolution in a one-shot deprecation warner. Each `(model, process)` pair emits `error_log("[SuperAgent] model 'X' is deprecated: retires YYYY-MM-DD (N days left) — switch to 'Y'.")` exactly once. `SUPERAGENT_SUPPRESS_DEPRECATION=1` (also accepts `true` / `yes` / `on`) silences. `ModelResolver::reset()` clears the warned-set for test isolation.

#### Catalog entries (resources/models.json)
- **`deepseek` provider block** — `env: DEEPSEEK_API_KEY`, `regions: ['default']`, with four model rows:
  - `deepseek-v4-pro` — `family: deepseek-v4`, `date: 20260424`, input `$0.55/M` / output `$2.20/M`, aliases `deepseek` / `deepseek-v4` / `v4-pro`, capabilities include `thinking: true`, `agentic: true`, `context_cache: auto`, `max_context: 1048576`, `active_params: 49B`, `total_params: 1.6T`
  - `deepseek-v4-flash` — `family: deepseek-v4`, `date: 20260424`, input `$0.14/M` / output `$0.55/M`, aliases `v4-flash` / `deepseek-flash`, `active_params: 13B`, `total_params: 284B`
  - `deepseek-chat` — `deprecated_until: 2026-07-24`, `replaced_by: deepseek-v4-flash`, description flags the retirement
  - `deepseek-reasoner` — `deprecated_until: 2026-07-24`, `replaced_by: deepseek-v4-pro`

### Fixed

- **OpenAI-compat cache billing was overcounting cached input by ~110%.** `ChatCompletionsProvider::parseSSEStream()` populated `Usage::inputTokens` from the gross `usage.prompt_tokens` field while also setting `Usage::cacheReadInputTokens` from `prompt_tokens_details.cached_tokens`; `CostCalculator::calculate()` then applied full input price to *all* prompt tokens *and* added 10% to the cached slice — billing the cached portion at 110% instead of 10%. The parser now subtracts the cached count from `prompt_tokens` before storing it as `inputTokens`, so the existing cost arithmetic produces the correct number. Affects every OpenAI-compat backend with cache reads (Kimi, OpenAI itself, DeepSeek, GLM where caching is supported); Anthropic's path is wire-correct and unaffected. `Usage::totalTokens()` still returns the same number (it sums all fields), so token counting elsewhere is unchanged.
- **`prompt_cache_hit_tokens` (DeepSeek's historical V3 cache wire) is now recognized.** Previously the parser only accepted `prompt_tokens_details.cached_tokens` (current OpenAI shape) and the legacy top-level `cached_tokens`; DeepSeek V3 / R1 emit the hit/miss split exclusively, and silently dropping the metric meant a 90%-cache-hit V3 chat conversation was billed at full price every turn. The parser now reads any of the three shapes; first non-zero wins.

### Migration path

Existing callers keep working. To use V4 directly:

```php
use SuperAgent\Agent;

// OpenAI-wire (default for the deepseek provider)
$agent = new Agent([
    'provider' => 'deepseek',
    'api_key'  => getenv('DEEPSEEK_API_KEY'),
    'model'    => 'deepseek-v4-pro',  // or 'deepseek-v4-flash'
]);
$agent->run('write a CLI tool that ...');

// Turn on V4's thinking channel — reasoning_content arrives as a
// separate ContentBlock(type=thinking) in the assistant turn.
$agent->run('hard reasoning prompt', options: ['thinking' => true]);

// Anthropic-wire over the same DeepSeek backend (no DeepSeekProvider needed):
$anthropicWire = new Agent([
    'provider' => 'anthropic',
    'api_key'  => getenv('DEEPSEEK_API_KEY'),
...
Users on `deepseek-chat` / `deepseek-reasoner` see a one-shot warning per process recommending the V4 replacement. To silence it (e.g. in CI):

```bash
export SUPERAGENT_SUPPRESS_DEPRECATION=1
```

### Notes

- **Both wire families are first-class.** `DeepSeekProvider` (OpenAI shape) is the typed entry point; the Anthropic-compat path is a configuration of `AnthropicProvider`. Pick whichever matches the rest of your conversation history wire — neither is "preferred".
- **Reasoning channel applies broadly.** The `delta.reasoning_content` plumbing in the shared base parser benefits every existing OpenAI-compat subclass (Kimi-thinking, Qwen-reasoning over the OpenAI mode, GLM-thinking, OpenAI's o-series via Chat Completions) without those classes needing changes. Providers that have always emitted reasoning text on the regular `content` channel are unaffected.
- **V4 thinking budget is server-controlled.** DeepSeek does not expose a `budget_tokens` field on the wire (unlike Anthropic). The advisory budget passed to `thinkingRequestFragment()` is currently ignored; V4-Pro thinks more aggressively than V4-Flash by tier, not by request flag. If DeepSeek adds a budget knob later, the fragment shape is forward-compatible.
- **Beta endpoint.** Setting `region: 'beta'` swaps the base URL to `https://api.deepseek.com/beta` for FIM / prefix completion access. The chat path is unchanged; only the host differs. Treat it as opt-in for code-completion workloads, not the default chat region.
- **`SuperAgentApplication` `VERSION` constant** bumped `0.9.5 → 0.9.6`.
...
**AgentTool productivity instrumentation — every sub-agent result now carries hard evidence of what the child actually did.** Fixes a recurring `/team` regression where orchestrators (notably MiniMax-backed ones) accepted `success: true` at face value and moved on, even though the child had produced only prose — no tool calls, no files, no consolidation. The runtime now observes each child's `tool_use` stream directly and returns `filesWritten`, `toolCallsByName`, `totalToolUseCount`, `productivityWarning`, plus a sharpened `status` field on every `AgentTool` result.

Scope is deliberately small: one file changed in `src/` (`Tools/Builtin/AgentTool.php`), one new test file, zero public signature changes, zero new provider contracts. Everything pre-0.8.9 keeps its exact shape — the new fields are additive.

### Added

#### `src/Tools/Builtin/AgentTool.php` — productivity accumulators

- **`recordToolUse(string $agentId, string $toolName, array $toolInput)`** — private hook called from `applyProgressEvents()` on every streamed `tool_use` event (both the canonical `assistant`-message path and the legacy `__PROGRESS__` `tool_use` event path). Increments per-name tool counters and, for the write-class tools (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`, `Create`), captures the `file_path` / `path` input so the orchestrator sees concrete paths, not just counts. Malformed tool_use blocks with no path are counted toward `toolCallsByName` but don't pollute `filesWritten`.
- **`buildProductivityInfo(string $agentId, int $childReportedTurns): array`** — single source of truth for the result block appended by both the process-pool branch (`waitForProcessCompletion()`) and the fiber branch (`waitForFiberCompletion()`). Returns the final `status`, deduped `filesWritten` list, `toolCallsByName` map, `totalToolUseCount` (prefers observed tool-use count over child-reported turn count — `turns` counts assistant turns, not tool calls), and `productivityWarning` string.
- **New accumulator fields on `$activeTasks[$agentId]`** — `tool_counts` (name→count) and `files_written` (list of absolute paths), populated during the stream and reset per dispatch.

#### New result fields on every `AgentTool` success payload

- `filesWritten: list<string>` — absolute paths the child wrote via `Write` / `Edit` / `MultiEdit` / `NotebookEdit` / `Create`. Deduped. Empty list when the child produced results via text only (legitimate for advisory consults).
- `toolCallsByName: array<string,int>` — e.g. `['Read' => 2, 'Bash' => 5, 'Write' => 1]`. Gives the orchestrator a precise picture of how the child spent its budget without having to scrape the child's narrative.
- `productivityWarning: ?string` — `null` on clean runs; an informational string when the child called tools but wrote no files; a stronger "zero tool calls" string when the child produced only prose.
- `totalToolUseCount` — now reflects **observed** tool calls when any were seen, falling back to child-reported turns only when nothing was observed. Pre-0.8.9 this field counted turns unconditionally.

#### Three productivity statuses

- `completed` — normal success. The child ran tools; files may or may not have been written (see `filesWritten`).
- `completed_empty` — **zero tool calls observed**. The model described what it would do instead of doing it. Orchestrators should always treat this as a dispatch failure: re-dispatch with a more explicit instruction, or pick a stronger model.
- `async_launched` — unchanged from 0.8.8. Only emitted when the caller passed `run_in_background: true`.

A fourth status — `completed_no_writes` — was briefly present during 0.8.9 development (flagging "called tools but wrote no files" as a failure). It has been deliberately removed: MiniMax-backed orchestrators over-read it as terminal failure and fell back to self-impersonation mid-run, producing a single rushed report and skipping consolidation. The no-writes case is now surfaced as an **advisory** `productivityWarning` while the status stays `completed` — callers decide whether files are required based on their own contract.

#### Expanded tool description + `run_in_background` doc

- `AgentTool::description()` now spells out the parallelism contract explicitly: "to run multiple agents concurrently, emit all agent calls as separate `tool_use` blocks in a single assistant message — the runtime fans them out in parallel and blocks until every child finishes." Also documents each of the three status values so orchestrators don't have to guess.
- The `run_in_background` schema description flips from one line to a full "fire-and-forget" clarification — background mode is wrong for any workflow that needs to consolidate child outputs; it returns `async_launched` immediately with no result to read. The right way to run agents in parallel is to emit multiple foreground calls in the same assistant message.

#### Tests (`tests/Unit/AgentToolProductivityTest.php`)

Five locked-down scenarios against the private `recordToolUse()` / `buildProductivityInfo()` surface (reflection — the public API stays the returned array):

- `testCompletedWithWrites` — Read + 2 Writes → `completed`, no warning, two paths in `filesWritten`, count map, observed-count beats child-reported turns.
- `testToolsRanButNoWritesStaysCompleted` — Bash + Read with no Writes → `completed` + **advisory** warning containing "wrote no files" and "advisory". Pins the RUN 72 regression fix.
- `testCompletedEmpty` — zero tool calls → `completed_empty` + "zero tool calls" warning; `totalToolUseCount` falls back to the child-reported turn count so downstream book-keeping still has a number.
- `testDuplicatePathsDeduped` — two Edits + one Write on the same path → single entry in `filesWritten`; per-name counts preserved.
- `testWriteToolWithoutPathDoesNotPollute` — malformed Write with no `file_path` → tool call counted but `filesWritten` stays empty; status is `completed` with the advisory warning.

### Changed

- `AgentTool::waitForProcessCompletion()` / `waitForFiberCompletion()` now call `buildProductivityInfo()` once at the tail and spread its result into the returned `ToolResult`. The old `'status' => 'completed'` literal is replaced by the computed status so `completed_empty` propagates to callers.
- `applyProgressEvents()` casts tool names and inputs to their declared types before both forwarding to `ProgressTracker::addToolActivity()` and calling `recordToolUse()`. No behaviour change for well-formed streams; defends against stray integer / null values on malformed ones.

### Why (context the code won't tell you)

Dispatching N sub-agents and trusting `success: true` from each has been reliable for Claude-Code-class brains but flaky for brains optimised on skill-adherence metrics rather than tool-use reliability — the model declares the plan done without actually firing tools. Logs from `/team` RUN 61 and RUN 72 both showed the same failure shape: the SuperAgent orchestrator accepted `success: true`, the child's final text was a prose plan, `filesWritten` (had it existed) would have been empty. The fix is observational, not heuristic: we count the tool_use events we actually saw and let the orchestrator decide what "productive" means for its task.

### Compat Red Lines (All Green)

- Zero public method signatures changed.
- `AgentTool` success payloads gain four fields (`filesWritten`, `toolCallsByName`, `productivityWarning`, and a sharper `totalToolUseCount`) — additive; callers that destructure known keys are unaffected.
- `status: 'completed'` still emits in the exact same conditions as pre-0.8.9 except the "zero tool calls" edge case, which now routes to `completed_empty`. No pre-0.8.9 test asserted `status === 'completed'` for a zero-tool-call run (the shape didn't exist in practice).
- Every other provider / router / security / MCP / Skills code path is byte-exact with 0.8.8.

**Test suite: 2476 tests / 6891 assertions / 0 failures** (up from 2471 / 6879 at 0.8.8 — 5 new `AgentToolProductivityTest` scenarios).
