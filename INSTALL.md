...
| Requirement | Minimum | Tested in CI |
|---|---|---|
...
| Requirement | Minimum | Tested in CI |
|---|---|---|
| Laravel | 10.0 | 10, 11, 12, 13 (one job per major, booting the service provider) |
...
```bash
superagent --version    # SuperAgent v1.1.10
superagent --help
```
...
Nothing to install beyond the standard package — `gemini-3.8-flash` / `gemini-3.7-flash` / `gemini-3.1-flash-lite` already live in the bundled `resources/models.json`. Set the key once:
...
The provider default is now `gemini-3.8-flash` (GA 2026-09-02, intro $0.75/$3.75 per 1M through 2026-12-31, then $1.50/$7.50; `thinking_level` is `low|medium|high` — `minimal` is rejected there and the SDK maps it to `low`); pass `--model gemini-3.1-pro-preview` for hardest tasks or `--model gemini-3.1-flash-lite` for cheapest.
...
The default Qwen model is now `qwen3.8-max-0902` (snapshot 2026-09-02: 1M context, $2/$6 per 1M tokens, multimodal, stronger engineering-scale coding and collaborative agents; `qwen3.8-max` and `qwen3.7-max` stay reachable, and the cheaper `qwen3.8-flash` / `qwen3.8-27b` tiers are in the catalog). Three provider keys access Qwen:
